 ...
 */
public class MyMainReporter implements IReporter {
	private static ResourceBundle _prop = ResourceBundle.getBundle("selenium");

	File reportDirectory = new File("../" + _prop.getString("jenkinsProjectName") + "/automation");//Just a test, could be uncommented
	

	private static final Logger L = Logger.getLogger(MyMainReporter.class);

	// ~ Instance fields ------------------------------------------------------
	final DateFormat df = new SimpleDateFormat("yyyy-MM-dd-HH-mm");
	final String now = df.format(new Date());
	public String testClassName;
	public String testReportTimeStamp = now + ".html";
	...
	public void generateReport(List<XmlSuite> xml, List<ISuite> suites, String outdir) {
		try {
			m_out = createWriter(xml, suites, outdir);
		} catch (IOException e) {
	...
	protected PrintWriter createWriter(List<XmlSuite> xml, List<ISuite> suites, String outdir) throws IOException {
	...
	protected void generateMethodSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				...
				Collection<ITestNGMethod> failedMap = testContext.getFailedTests().getAllMethods();

				Collection<ITestNGMethod> passedMap = testContext.getPassedTests().getAllMethods();

				for (ITestNGMethod ft : failedMap) {

					for (ITestNGMethod fp : passedMap) {
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests)) {
				...
				if (!className.equalsIgnoreCase(lastClassName)) {
					if (mq > 0) {
						cq += 1;
						m_out.println("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td rowspan=\"" + mq + "\">" + lastClassName + buff);
					}
					mq = 0;
					buff.setLength(0);
				...
				buff.append("<td><a href=\"#m" + m_methodIndex + "\">" + qualifiedName(method) + " " + (description != null && description.length() > 0 ? "(\"" + description + "\")" : "") + "</a>" + (null == testInstanceName ? "" : "<br>(" + testInstanceName + ")") + "</td>" + "<td class=\"numi\">" + resultSet.size() + "</td><td class=\"numi\">" + (end - start) + "</td></tr>");
			}
			if (mq > 0) {
				cq += 1;
				m_out.println("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td rowspan=\"" + mq + "\">" + lastClassName + buff);
	...
	private void startResultSummaryTable(String style) {
		tableStart(style);
		m_out.println("<tr><th>Class</th>" + "<th>Method</th><th># of<br/>Tries</th><th>Time<br/>(Msecs)</th></tr>");
		m_row = 0;
	}
	...
	private void resultDetail(IResultMap tests, final String style) {
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests)) {
				...
				m_out.println("<a id=\"m" + m_methodIndex + "\"></a><h2>" + cname + ":" + method.getMethodName() + "</h2>");
				int rq = 0;
				Set<ITestResult> resultSet = tests.getResults(method);
				for (ITestResult ans : resultSet) {
					...
					if (hasParameters) {
						if (rq == 1) {
							...
							for (int x = 1; x <= parameters.length; x++) {
								m_out.print("<th style=\"padding-left:1em;padding-right:1em\">Parameter #" + x + "</th>");
							}
							m_out.println("</tr>");
						}
						m_out.print("<tr" + (rq % 2 == 0 ? " class=\"stripe\"" : "") + ">");
						for (Object p : parameters) {
							m_out.println("<td style=\"padding-left:.5em;padding-right:2em\">" + (p != null ? Utils.escapeHtml(p.toString()) : "null") + "</td>");
						}
						m_out.println("</tr>");
					}
					...
					if (hasReporterOutput || hasThrowable) {
						...
						if (hasParameters) {
							m_out.println("<tr" + (rq % 2 == 0 ? " class=\"stripe\"" : "") + "><td" + indent + " colspan=\"" + parameters.length + "\">");
						} else {
	...
	private void tableStart(String cssclass) {
		m_out.println("<table cellspacing=0 cellpadding=0" + (cssclass != null ? " class=\"" + cssclass + "\"" : " style=\"padding-bottom:2em\"") + ">");
		m_row = 0;
	}
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println("</script>");
		out.println("<script src=\"/lib/new3.js\">");
		out.println("</script>");
	...
	private class TestSorter<T extends ITestNGMethod> implements Comparator {
		...
		public int compare(Object o1, Object o2) {
			...
			testClassName = ((T) o1).getTestClass().getName();
			if (r == 0) {
		...
		protected PrintWriter createWriter(String outdir) throws IOException {
			...
			return new PrintWriter(new BufferedWriter(new FileWriter(new File(reportDirectory, testReportTimeStamp))));
